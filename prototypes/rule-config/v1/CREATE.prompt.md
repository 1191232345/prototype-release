...
3. 阶段一：澄清需求，确认语为 **「需求理解正确，继续细化」**
4. 阶段二：整理原型方案，确认语为 **「理解正确，继续创建」**
...
| 目录 slug | rule-config |
| 版本 | v1 |
| 目标目录 | prototypes/rule-config/v1/ |

## 参考项目（请先 Read 理解结构，勿复制全文到对话）

| 文件 | 路径 |
|------|------|
| Flow 骨架 | `prototypes/rule-config/v1/flow.json` |
| 列表 Spec | `prototypes/rule-config/v1/pages/list.json` |
| 表单 Spec | `prototypes/rule-config/v1/pages/form.json` |
| PRD | `prototypes/rule-config/v1/PRD.md` |
...
输出最后一行必须是：
```
✋ 需求理解已完成。若理解正确，请回复「需求理解正确，继续细化」；若有误请直接指出需修正之处，我会更新后再请您确认。
```
...
输出最后一行必须是：
```
✋ 原型方案已整理。若方案正确，请回复「理解正确，继续创建」；若有调整请直接说明，我会修订后再请您确认。
```
...
7. 运行 `npm run prompts:gen -- rule-config/v1` 同步 Prompt 文件
