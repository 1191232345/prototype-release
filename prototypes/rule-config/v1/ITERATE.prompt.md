...
| 优先级 | 文件 | 说明 |
|--------|------|------|
| 1 | `prototypes/rule-config/v1/flow.json` | 入口页、pages 列表 |
| 2 | `prototypes/rule-config/v1/pages/*.json` | 各页 Spec（含 `shared.sections.*.json`、`*.details.json` 等片段） |
| 3 | `prototypes/rule-config/v1/PRD.md` | 评审对照真源（第 3 章锚点） |
| 4 | `prototypes/rule-config/v1/REQUIREMENTS.md` | 需求与变更记录 |
| 5 | `prototypes/rule-config/v1/meta.json` | changeSummary、status |
| 6 | `prototypes/rule-config/v1/CHANGELOG.md` | 历史变更 |
...
6. 运行 `npm run acceptance:gen -- rule-config/v1`
7. 若 Prompt 规则有变，运行 `npm run prompts:gen -- rule-config/v1`
