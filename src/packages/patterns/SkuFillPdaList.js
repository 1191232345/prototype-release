...
function PdaTaskCard({ row, details, onOpen, }) {
    ...
    const isCompleted = row.status === 'completed' || statusText === '已完结';
    const hasSku = Boolean(sku.name);
    const cardTitle = hasSku ? sku.name : orderNo;
    const cardSubtitle = hasSku ? sku.code : '';
    return (_jsx("li", { children: _jsxs("button", { type: "button", className: "w-full text-left bg-white rounded-xl border border-border-light shadow-sm p-3 active:bg-light-bg transition-colors", onClick: onOpen, ...reviewTarget(`list.row.${row.id}`, cardTitle), children: [_jsxs("div", { className: "flex items-start justify-between gap-2 mb-2", children: [_jsxs("div", { className: "min-w-0", children: [_jsx("p", { className: "text-sm font-semibold text-dark truncate", children: cardTitle }), cardSubtitle ? (_jsx("p", { className: "text-xs text-text-muted truncate mt-0.5", children: cardSubtitle })) : null] }), _jsx(SkuFillStatusBadge, { label: statusText })] }), (hasSku && orderNo !== '—') || warehouse !== '—' ? (_jsxs("div", { className: "flex flex-wrap items-center gap-2 text-[11px] text-text-secondary mb-2", children: [hasSku && orderNo !== '—' ? (_jsxs("span", { className: "inline-flex items-center gap-1", children: [_jsx(FaIcon, { className: "fas fa-clipboard-list text-text-muted" }), orderNo] })) : null, warehouse !== '—' ? (_jsxs("span", { className: "inline-flex items-center gap-1", children: [_jsx(FaIcon, { className: "fas fa-warehouse text-text-muted" }), warehouse] })) : null] })) : null, _jsxs("div", { className: "flex items-center justify-between pt-2 border-t border-border-light", children: [_jsx("span", { className: "text-xs text-accent font-medium", children: isCompleted ? '查看详情' : '开始录入' }), _jsx(FaIcon, { className: "fas fa-chevron-right text-accent text-xs" })] })] }) }));
}
