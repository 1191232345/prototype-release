...
function ConfigSectionCard({ section, rows, onRowsChange, }) {
    ...
    return (_jsxs(Card, { title: section.title, icon: section.icon, children: [section.zonePriceTable && (_jsx("div", { ...reviewTarget('form.zone.table'), children: _jsx(ZonePriceTableEditor, { table: section.zonePriceTable }) })), section.surchargeTable && (_jsx("div", { ...reviewTarget(section.title.includes('其他') ? 'form.other.table' : 'form.surcharge.table'), children: _jsx(SurchargeTableEditor, { table: section.surchargeTable, rows: rows?.[key], onRowsChange: onRowsChange ? (next) => onRowsChange(key, next) : undefined }) }))] }));
}
