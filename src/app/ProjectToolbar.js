...
import { canPublishProject, isLocalPendingDraft } from '../lib/projectStore';
...
export function ProjectToolbar({ project, immersive = false, publishConfigured = false, publishing = false, onMenuClick, onEdit, onDelete, onIterate, onPublish, }) {
    ...
    const showPublish = canPublishProject(project);
    return (_jsx("header", { className: "shrink-0 bg-white border-b border-border-light relative z-30", children: _jsxs("div", { className: "px-3 sm:px-5 py-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3", children: [_jsxs("div", { className: "flex items-center gap-1 min-w-0 flex-1", children: [_jsx("button", { type: "button", className: `toolbar-icon-btn shrink-0 -ml-1 ${immersive ? '' : 'lg:hidden'}`, title: "\u6253\u5F00\u9879\u76EE\u5217\u8868", onClick: onMenuClick, children: _jsx(FaIcon, { className: "fas fa-bars" }) }), !isPendingDraft && !isPendingOnDisk ? (_jsx("p", { className: "text-sm font-semibold text-dark truncate max-w-[120px] sm:max-w-[240px] mr-1 shrink-0 hidden lg:block", children: project.meta.title })) : isPendingDraft ? (_jsxs("p", { className: "text-xs sm:text-sm text-amber-700 truncate flex-1", children: [_jsx(FaIcon, { className: "fas fa-hourglass-half mr-1 opacity-70" }), _jsx("span", { className: "hidden sm:inline", children: "\u5F85\u521B\u5EFA \u00B7 \u590D\u5236 Prompt \u540E\u5728 Cursor \u4E2D\u5B8C\u6210\u5F52\u6863" }), _jsx("span", { className: "sm:hidden", children: "\u5F85\u521B\u5EFA" })] })) : (_jsxs("p", { className: "text-xs sm:text-sm text-amber-700 truncate flex-1", children: [_jsx(FaIcon, { className: "fas fa-triangle-exclamation mr-1 opacity-70" }), _jsx("span", { className: "hidden sm:inline", children: "\u5F85\u5B8C\u5584 \u00B7 \u8865\u5168 Spec \u540E\u53EF\u53D1\u5E03\u9884\u89C8" }), _jsx("span", { className: "sm:hidden", children: "\u5F85\u5B8C\u5584" })] }))] }), _jsx(ProjectActions, { project: project, isPendingDraft: isPendingDraft, showPublish: showPublish, publishConfigured: publishConfigured, publishing: publishing, onEdit: onEdit, onDelete: onDelete, onIterate: onIterate, onPublish: onPublish })] }) }));
}
function ProjectActions({ project, isPendingDraft, showPublish, publishConfigured, publishing, onEdit, onDelete, onIterate, onPublish, }) {
    ...
    return (_jsxs("div", { className: "flex items-center gap-1 shrink-0 self-end sm:self-auto", children: [showPublish && onPublish && (_jsxs("button", { type: "button", className: `btn py-1.5 px-2.5 sm:px-3 text-sm gap-1.5 ${publishConfigured ? 'btn-secondary' : 'btn-secondary ring-1 ring-amber-300/80'}`, disabled: publishing, title: publishConfigured
                    ? '查看或更新预览链接（内容未变更时不会重复上传）'
                    : '点击配置发布服务器（需填写 Access Token）', onClick: onPublish, children: [_jsx(FaIcon, { className: `fas ${publishing ? 'fa-spinner fa-spin' : 'fa-link'}` }), _jsx("span", { className: "hidden sm:inline", children: publishing ? '加载中…' : '预览链接' })] })), _jsxs("button", { type: "button", className: "btn btn-primary py-1.5 px-2.5 sm:px-3 text-sm gap-1.5", onClick: onIterate, children: [_jsx(FaIcon, { className: "fas fa-code-branch" }), _jsx("span", { className: "hidden sm:inline", children: "\u8FED\u4EE3\u539F\u578B" })] }), _jsx(MoreMenu, { menuRef: moreRef, open: moreOpen, onToggle: () => setMoreOpen((v) => !v), items: [
                    { icon: 'fa-edit', label: '编辑信息', onClick: onEdit },
                    { icon: 'fa-trash', label: '删除项目', danger: true, onClick: onDelete },
                ] })] }));
}
