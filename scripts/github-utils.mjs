/** GitHub / Gitee 发布相关工具（移植自 requirement-factory） */
...
export function parseGithubRepo(repoUrl) {
  ...
  if (!match) throw new Error('无法从 GitHub 仓库地址解析 owner/repo');
...
export function githubPublishMissing(data) {
  ...
  if (!repo) missing.push('仓库地址');
  if (!token) missing.push('Personal Access Token');
  if (!pages) missing.push('Pages 预览前缀');
  return missing;
}
