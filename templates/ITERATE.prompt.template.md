...
| 优先级 | 文件 | 说明 |
|--------|------|------|
| 1 | `{{DIR}}/flow.json` | 入口页、pages 列表 |
| 2 | `{{DIR}}/pages/*.json` | 各页 Spec（含 `shared.sections.*.json`、`*.details.json` 等片段） |
| 3 | `{{DIR}}/PRD.md` | 评审对照真源（第 3 章锚点） |
| 4 | `{{DIR}}/REQUIREMENTS.md` | 需求与变更记录 |
| 5 | `{{DIR}}/meta.json` | changeSummary、status |
| 6 | `{{DIR}}/CHANGELOG.md` | 历史变更 |
...
7. 若 Prompt 规则有变，运行 `npm run prompts:gen -- {{SLUG}}/{{VERSION}}`
