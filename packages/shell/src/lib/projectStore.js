...
import { generateProjectPrompt } from './prompt';
...
export function saveMetaOverride(key, title, description, status, tags) {
...
export function setProjectTags(key, tags) {
    const map = loadOverrides();
    const prev = map[key];
    map[key] = {
        key,
...
export function recordProjectOpened(key) {
    const map = loadOverrides();
    const prev = map[key];
    if (!prev) {
        map[key] = {
...
export function saveProjectInfo(project, name, description, status, tags) {
    if (project.status === 'pending') {
        updatePendingProject(project.key, name, description);
    }
    else {
        saveMetaOverride(project.key, name, description, status ?? getProjectStatus(project), tags);
    }
}
