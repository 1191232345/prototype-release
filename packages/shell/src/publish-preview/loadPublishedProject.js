...
import { assemblePublishedPageSpec } from './assemblePublishedPage';
...
export async function loadPublishedProject() {
    ...
    for (const pageId of manifest.pages) {
        const rawPage = await fetchJson(`${base}pages/${pageId}.json`);
        const page = await assemblePublishedPageSpec(base, pageId, rawPage);
        pageMap[pageId] = page;
        try {
