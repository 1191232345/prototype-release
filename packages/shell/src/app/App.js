...
import { Toast } from '@prototype/ui';
import { FaIcon } from '@prototype/ui/Icon';
import { copyIteratePrompt } from '../lib/prompt';
import { countValidationErrors, invalidateProjectsCache, loadProjectDetail, loadProjectsWithReconcile } from '../data/prototypes';
import { canPublishProject, isLocalPendingDraft, recordProjectOpened } from '../lib/projectStore';
...
export function App() {
    ...
    const lastValidateErrorCountRef = useRef(0);
    ...
    useEffect(() => {
        ...
        void loadProjectDetail(effectiveKey)
            .then((detail) => {
            if (!cancelled) {
            ...
            if (!cancelled) {
                setPreviewProject(listed ?? null);
                setDetailLoading(false);
                showToast('加载项目详情失败，请刷新重试', 5000);
            }
        });
        return () => {
            cancelled = true;
        };
    }, [effectiveKey, listRefreshKey, previewRefreshKey, projects, showToast]);
    useEffect(() => {
        if (!import.meta.hot)
            return;
        const refresh = () => {
        ...
        import.meta.hot.accept('../data/prototypes.ts', refresh);
        import.meta.hot.accept('../data/prototypeGlob.ts', refresh);
    }, [handleListRefresh]);
    useEffect(() => {
        if (!import.meta.env.DEV)
            return;
        const errCount = countValidationErrors(projects);
        if (errCount > 0 && errCount !== lastValidateErrorCountRef.current) {
            lastValidateErrorCountRef.current = errCount;
            showToast(`发现 ${errCount} 个 Meta 校验问题`, 6000);
        }
        else if (errCount === 0) {
            lastValidateErrorCountRef.current = 0;
        }
    }, [projects, showToast]);
    useEffect(() => {
        if (!sidebarOpen)
    ...
    return (_jsxs("div", { className: "h-screen flex overflow-hidden bg-surface", children: [_jsx(Sidebar, { projects: projects, selectedKey: effectiveKey, search: search, open: sidebarOpen, overlay: immersive, onSearchChange: setSearch, onSelect: (key) => {
                    ...
                    setShowSettings(true);
                    setSidebarOpen(false);
                }, onRefresh: handleListRefresh, onClose: () => setSidebarOpen(false), onLocalChange: handleListRefresh, onEditProject: () => setShowEdit(true), onDeleteProject: () => setShowDelete(true) }), _jsxs("main", { className: "flex-1 flex flex-col min-w-0 h-full w-full", children: [_jsx(ProjectToolbar, { project: previewItem, onMenuClick: () => setSidebarOpen(true), immersive: immersive }), _jsx(PreviewPanel, { project: previewItem, loading: detailLoading, onPrdSaved: handlePrdSaved }), previewItem && !isLocalPendingDraft(previewItem) && (_jsx(FloatingActions, { onIterate: () => void handleIterate(), onPublish: canPublishProject(previewItem) ? () => void handleOpenPublish() : undefined, publishConfigured: Boolean(publishConfig?.publishConfigured), publishing: publishLoading || publishing }))] }), _jsx(ProjectPromptModal, { open: showCreate, platform: createPlatform, onClose: () => setShowCreate(false), onDone: handlePromptDone }), _jsx(EditProjectModal, { project: selected, allProjects: projects, open: showEdit, onClose: () => setShowEdit(false), onSaved: (key) => {
                    handleListRefresh();
                    setSelectedKey(key);
...
function FloatingActions({ onIterate, onPublish, publishConfigured, publishing, }) {
    return (_jsxs("div", { className: "fixed bottom-5 right-5 z-30 flex flex-col gap-2", children: [onPublish && (_jsxs("button", { type: "button", className: `group flex items-center gap-2 px-3.5 py-2.5 rounded-full shadow-lg border transition-all hover:shadow-xl active:scale-95 ${publishConfigured
                    ? 'bg-white text-text-secondary border-border-light hover:bg-hover'
                    : 'bg-amber-50 text-amber-700 border-amber-300/80 hover:bg-amber-100'}`, disabled: publishing, title: publishConfigured ? '查看或更新预览链接' : '点击配置发布服务器', onClick: onPublish, children: [_jsx(FaIcon, { className: `fas ${publishing ? 'fa-spinner fa-spin' : 'fa-cloud-upload-alt'} text-sm` }), _jsx("span", { className: "text-sm font-medium", children: publishing ? '加载中…' : '发布' })] })), _jsxs("button", { type: "button", className: "group flex items-center gap-2 px-3.5 py-2.5 rounded-full bg-primary text-white shadow-lg border border-primary transition-all hover:shadow-xl hover:bg-primary-dark active:scale-95", title: "\u8FED\u4EE3\u539F\u578B", onClick: onIterate, children: [_jsx(FaIcon, { className: "fas fa-code-branch text-sm" }), _jsx("span", { className: "text-sm font-medium", children: "\u8FED\u4EE3\u539F\u578B" })] })] }));
}
