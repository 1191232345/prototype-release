...
export function deriveFormPageExpectations(page) {
    ...
    for (const section of page.sections ?? []) {
        ...
        if (section.feeItemField) {
            const feeItemReviewId = section.title.includes('其他')
                ? 'form.other.field.feeItem'
                : 'form.surcharge.field.feeItem';
            out.push({
                reviewId: feeItemReviewId,
                anchorId: section.title.includes('其他') ? 'form.other-table' : 'form.surcharge-table',
                rowLabel: section.feeItemField.label,
                source: `feeItemField.${section.feeItemField.id}`,
            });
        }
        if (section.zonePriceTable) {
            out.push({
                ...
                reviewId: 'form.zone.btn.delete',
                anchorId: 'form.zone-table',
                rowLabel: '删除重量段',
                source: 'zoneButton.delete',
            }, {
    ...
    if (page.warehouseOptions?.length) {
        out.push({
            reviewId: 'form.warehouse.chips',
            anchorId: 'form.warehouse-split',
            rowLabel: '适用仓库',
            source: 'warehouse.chips',
        }, {
            ...
            reviewId: 'form.warehouse.select-all',
            anchorId: 'form.warehouse-split',
            rowLabel: '全选',
            source: 'warehouse.select-all',
        }, {
            reviewId: 'form.warehouse.clear-all',
            anchorId: 'form.warehouse-split',
            rowLabel: '取消全选',
            source: 'warehouse.clear-all',
        }, {
            reviewId: 'form.fee.empty',
            anchorId: 'form.quick-fill',
            rowLabel: '费用空状态',
            source: 'fee.empty',
        }, {
            reviewId: 'form.fee.manual',
            anchorId: 'form.interaction',
            rowLabel: '手动配置',
            source: 'fee.manual',
        }, {
            reviewId: 'form.fee.progress',
