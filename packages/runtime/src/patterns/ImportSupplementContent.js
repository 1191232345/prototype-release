...
import { Button, Card, Toast, useToast } from '@prototype/ui';
...
export function ImportSupplementContent({ tableSpec, asyncThreshold = DEFAULT_THRESHOLD, filePresets, onCancel, onComplete, }) {
    ...
    const handleConfirmImport = () => {
        if (!fileName.trim()) {
            showToast('请先选择承运商账单文件');
            return;
        }
        if (detectedRows > asyncThreshold) {
            showToast(`文件共 ${detectedRows} 条，已超过 ${asyncThreshold} 条，已添加到异步任务`, 2000);
            setTimeout(() => onComplete('async'), 2000);
            return;
        }
    ...
    const handleConfirmCalc = () => {
        ...
        if (missing) {
            showToast(`请为客户 ${missing.customerCode} 选择价卡`);
            return;
        }
        showToast('差价计算完成，已生成补收记录', 1500);
        setTimeout(() => onComplete('sync'), 1500);
    };
